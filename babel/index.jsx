...
$(document).ready(() => {
  ...
  // Toggle search button and box
  function toggleSearch(enable) {
    // Restore search button and box to original state
    if (enable) {
      $sIcn.removeClass('fa-circle-o-notch fa-spin').addClass('fa-search');
      $sBtn.removeAttr('disabled');
      $sBox.removeAttr('disabled');
    } else {
      // Change search icon to animated spinner, and disable button and box
  ...
  $sBtn.mousedown(() => $sBtn.click()); // Handle mousedown events as clicks
  $sBtn.click(() => {
    // First, update UI
    $sBox.blur();
    toggleSearch();
